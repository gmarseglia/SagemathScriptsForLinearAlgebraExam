...
def declare_variables():
    ...
    if symbolic_names != "":
        ...
        var(symbolic_names, domain='real')
...
if use_gauss_reduction:
    MFinal = gauss_method(M)
    double_print("MFinal (using gauss_method)", MFinal)
else:
    if use_rref_instead_of_echelon_form:
        MFinal = M.rref()
        double_print("MFinal (using .rref())", MFinal)
    else:
        MFinal.echelonize()
        double_print("MFinal (using .echelonize())", MFinal)
